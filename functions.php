 ...
 *
 * @param array $less_files Required. array( $key => $value )
 * 										where $key is the path to the file to compile
 * 										and $value the url of the file
 * @param array $options Optional. Array of options.
 * @param array $modify_vars Optional. Array of modified vars.
 * @param string $cache_dir Optional. Path to cache directory. Default /css/cache in your theme
 * @param bool $delete_garbage Optional. Delete all the garbage generated by less.php. Default True
 *
 * @return string Name of the compiled file
 *
 * @since Twenty'em 1.0
 */
function t_em_lessphp_compiler( $less_files, $options = array(), $modify_vars = array(), $cache_dir = null, $delete_garbage = true ){
	require_once T_EM_THEME_DIR_INC_PATH . '/less-php/Autoloader.php';
	Less_Autoloader::register();

	if ( is_child_theme() ) :
		$dir		= '/css/cache/';
		$dir_path	= T_EM_CHILD_THEME_DIR_PATH . $dir;
		$dir_url	= T_EM_CHILD_THEME_DIR_URL . $dir;
	else :
		$dir		= '/cache/';
	...
	if ( ! isset( $cache_dir ) ) :
		$cache_dir = ( ! file_exists( $dir_path ) ) ? $cache_dir = mkdir( $dir_path ) : $dir_path;
	else :
		$cache_dir = ( ! file_exists( $cache_dir ) ) ? mkdir( $cache_dir ) : $cache_dir;
	endif;
	...
	$path = str_replace( bloginfo( 'url' ), '', T_EM_THEME_DIR_FONTS_URL .'/' );
	$cache_vars = array(
		'icon-font-path' => "'$path'",
	);
